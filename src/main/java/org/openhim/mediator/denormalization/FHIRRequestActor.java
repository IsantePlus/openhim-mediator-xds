...
public class FHIRRequestActor extends UntypedActor {
	...
	private void sendFHIRRequest(ResolvePatientIdentifier msg) {
		...
		MediatorHTTPRequest request = new MediatorHTTPRequest(
				...
				config.getProperty("fhir.mpiBaseUrl"), Integer.parseInt(config.getProperty("fhir.mpiPort")), "fhir/Patient",
				(String) null, headers, null
		);
		...
		try {
			...
			Bundle bundle = (Bundle) client.search().forResource(Patient.class)
					.where(new TokenClientParam("identifier").exactly()
							.systemAndCode(assigningAuthority, assigningAuthorityId + "/" + patientId))
					.execute();
	...
	public void onReceive(Object msg) throws Exception {
		if (msg instanceof ResolvePatientIdentifier) {
			log.info("Received request to resolve patient identifier in the '" + ((ResolvePatientIdentifier) msg)
					.getIdentifier().getAssigningAuthority().getAssigningAuthority() + "' domain(Assigning authority)");
			log.info("Received request to resolve patient identifier in the '" + ((ResolvePatientIdentifier) msg)
					.getIdentifier().getAssigningAuthority().getAssigningAuthorityId() + "' domain(Assigning authority Id)");
			log.info("Received request to resolve patient identifier in the '" + ((ResolvePatientIdentifier) msg)
					.getIdentifier().getAssigningAuthority().getAssigningAuthorityIdType()
					+ "' domain(Assigning authority type)");
